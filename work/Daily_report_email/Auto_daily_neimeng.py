...
START_TIME = '2020-01-12 00:00:00'  # 活动开始时间
PATH = r'C:\Users\Healthlink\Desktop\日报统计数据\内蒙0107\数据源\\'
EVENT_NAME = '2020“鼠”你幸福'
print('今天日期为：', TIME, '  星期', datetime.datetime.now().isoweekday(), sep='')

# 文件名称
FILE_sales = r'%s营销员会议-'%EVENT_NAME + str(TIME)+ ".csv"
FILE_member = r'2020“鼠”你幸福会员会议-' + str(TIME)+ ".csv"
...
df_card_info = df_card_info.drop_duplicates(subset='卡号', keep='first')  # 未知原因导致匹配后存在重复数据
df_card_info = df_card_info.reset_index(drop=True)  # 重置索引

writer = pd.ExcelWriter('卡激活匹配.xlsx')
df_card.to_excel(writer, sheet_name='原卡', index=False)
df_card_info.to_excel(writer, sheet_name='匹配', index=False)
df_card['机构'] = df_card_info['二级名称']
df_card.to_excel(writer, sheet_name='激活', index=False)
writer.save()
print('完成处理：<卡激活表>')
...
for company in company_count.index:
    writer = pd.ExcelWriter(PATH + '..//%s活动日报数据'%company + TIME + '.xlsx')
    # 按公司分别筛选表
    ...
    # send_email.send_mail(PATH, company, EVENT_NAME)

print('开始写入汇总数据')
writer = pd.ExcelWriter(PATH + '..//汇总活动日报数据' + TIME + '.xlsx')
...
#     send_email.summary_mail(PATH, EVENT_NAME)
